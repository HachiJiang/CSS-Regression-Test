          ...
          casper.waitFor(
            ...
            function() {
              ...
              if (_failures) {
                ...
                failFile = _failures + fs.separator + file.split(/\/|\\/g).pop().replace('.png', '') + '.fail.png';
...
function compareAll(exclude, list) {
  ...
  _diffsToProcess.forEach(function(file) {
    var baseFile = file.replace(_results, _src);
        ...
        tests.forEach(function(test) {
          if (test.fail) {
           ...
           /* var filename = test.filename;
            copyAndReplaceFile(filename.replace(_src, _results), filename.replace(_src, _failures));*/
            errors++;
          }
        });
...
function _onComplete(tests, noOfFails, noOfErrors) {
  ...
  // generate reports as html
  report.generateHTML(tests, noOfFails, noOfErrors);
  ...
  if (tests.length === 0) {
    ...
    if (noOfErrors !== 0) {
      console.log("There were " + noOfErrors + " errors.  Is it possible that a baseline image was deleted?");
    }
